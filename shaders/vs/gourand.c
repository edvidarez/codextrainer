...
vec3 clamp_v(vec3 v, float minVal, float maxVal)
{
  return vec3(min(max(v[0], minVal), maxVal),min(max(v[1], minVal), maxVal),min(max(v[2], minVal), maxVal));
}

float clamp_f (float x, float minVal, float maxVal)
{
...
void main() {
  ...
  //float factorD = clamp_f(  , 0, 1  );
  float factorD = min(max( dot(l, n) , 0.0), 1.0);
  
  vertexColorVF = ambientLight *  materialA + diffuseLight * materialD * factorD;
